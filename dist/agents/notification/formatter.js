...
class NotificationFormatter {
    ...
    format(command, channel) {
        const { eventType, orchestratorTimestamp, affectedServiceName, outcome, onCallHandle, } = command;
        const isEscalation = outcome === "escalated" ||
        ...
        // ── Build text fallback ─────────────────────────────────────────────
        let text = `[${outcome.toUpperCase()}] ${affectedServiceName} — ${eventType}` +
            ` | ${orchestratorTimestamp}`;
        if (isEscalation) {
            if (onCallHandle) {
                text += ` | ${onCallHandle}`;
